# Project Setup
PROJECT_NAME := configuration-caas-byoc
...
XPKG_IGNORE = .github/workflows/ci.yaml,.github/workflows/tag.yml,.github/workflows/e2e.yaml,.up/examples/local/*.yaml,.up/examples/*.yaml,.work/uptest-datasource.yaml,test/values.yaml,mcp-kubeconfig.yaml
...
uptest: $(UPTEST) $(KUBECTL) $(KUTTL) $(UP)
	@$(INFO) running automated tests
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) UPCLI=$(UP) $(UPTEST) e2e "${UPTEST_EXAMPLE_LIST}" --setup-script=test/setup.sh --default-timeout=3200 || $(FAIL)
